...
namespace SplineMesh {
    ...
    public class ExampleSower : MonoBehaviour {
        ...
        public void Sow() {
            ...
            while (distance <= spline.Length)
            {
                ...
                go.transform.localScale = 
                    new Vector3(RandomScale(scaleOffset.x),
                                RandomScale(scaleOffset.y),
                                RandomScale(scaleOffset.z));
                ...
                else
                {
                    go.transform.localEulerAngles =
                        sample.Rotation.eulerAngles + rotationOffset;
                }

                // move along spline, according to spacing + random
                go.transform.localPosition = sample.location
                    + go.transform.right * positionOffset.x
                    + go.transform.forward * positionOffset.z
                    + go.transform.up * positionOffset.y;
        ...
        private float RandomScale(float var)
        {
            return var + UnityEngine.Random.Range(0, scaleRange);
        }
    }
}
